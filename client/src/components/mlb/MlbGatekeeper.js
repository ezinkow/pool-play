...
export default function MlbGatekeeper({ user, children, isAdmin }) {
    ...
    useEffect(() => {
        ...
        axios.get(`/api/mlb/entries/check/${user.name}`)
            .then(res => {
    ...
    // 4. Logged in but no entry yet (Switched from old user.hasMlbEntry dependency tracking matrix)
    if (!hasEntry) {
        return (
            <>
