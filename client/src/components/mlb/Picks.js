...
import MlbGatekeeper from "./MlbGatekeeper";
...
export default function Picks() {
    ...
    useEffect(() => {
        if (authUser && series.length > 0) {
            ...
            axios.get("/api/mlb/picks", { params: { name: authUser.name }, ...config })
                .then(res => {
            ...
            axios.get("/api/mlb/tiebreaker", { params: { name: authUser.name }, ...config })
                .then(res => {
                    if (res.data) {
